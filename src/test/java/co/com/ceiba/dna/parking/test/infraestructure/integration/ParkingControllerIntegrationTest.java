...
public class ParkingControllerIntegrationTest {
	...
	public void checkInVehicleTest() throws Exception {
		...
		mvc.perform(post("/parking/checkIn/" + DEFAULT_ENTRY_DATE).contentType(MediaType.APPLICATION_JSON_UTF8)
				.content(vehicleJson)).andDo(print()).andExpect(status().is2xxSuccessful());
	}

	@Test
	public void calculatePrice() throws Exception {
		mvc.perform(get("/parking/price/" + DEFAULT_TICKET_ID).contentType(MediaType.APPLICATION_JSON_UTF8)).andDo(print())
				.andExpect(status().is2xxSuccessful());
	}

	@Test
	public void checkOutVehicle() throws Exception {
		mvc.perform(get("/parking/pay/" + DEFAULT_TICKET_ID).contentType(MediaType.APPLICATION_JSON_UTF8)).andDo(print())
		.andExpect(status().is2xxSuccessful());
	}
